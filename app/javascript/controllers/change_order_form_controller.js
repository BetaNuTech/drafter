...
export default class extends Controller {
  static targets = ['file_field', 'file_selected_image', 'submit_button', 'file_upload_prompt', 'amount_field', 'source_field', 'document', 'form']

  connect() {
    const submit_button_el = this.submit_buttonTarget
    if (this.file_present() && this.valid_amount() && this.valid_source()) {
  ...
  file_changed(){
    ...
    if (this.file_present()) {
      ...
      if (this.valid_amount() && this.valid_source()) {
        submit_button_el.removeAttribute('disabled')
      }
    } else {
      submit_button_el.setAttribute('disabled', true)
  ...
  valid_source() {
    const source_field_el = this.source_fieldTarget
    const source = source_field_el.value
    return(source != '')
  }
